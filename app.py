...
try:
    ...
    def upload():
        ...
        if request.method == 'POST':
            ...
            if 'scriptFile' in request.files: #For the script (if the user enters one)
                ...
                if scriptFile and scriptFile.filename != '':
                    ...
                    scriptFile.save(script_path)
                    with open(script_path, 'r', encoding='utf-8') as f:
                        references = [[f.read()]]
                else:
                    references = [[None]]

            video = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
            import cv2
            left_wrist_coords = []
            right_wrist_coords = []
            max_x = 0
            ...
            if references[0][0] != None:
                ...
                pdf.multi_cell(0, LINE_HEIGHT + BOX_PADDING, f"What was heard")
                pdf.set_font("Arial", "B", P_SIZE)
                if textSpeech == '':
                    pdf.set_font("Arial", "", P_SIZE)
                    pdf.multi_cell(0, SMALL_LINE_HEIGHT + BOX_PADDING, f"No words detected. Try talking next time")
                else:
                    pdf.set_font("Arial", "", P_SIZE)
                ...
                pdf.set_font("Arial", "B", P_SIZE)
                pdf.multi_cell(0, SMALL_LINE_HEIGHT + BOX_PADDING, f"{round(score,2)}%")
            ...
            pdf.set_font("Arial", "", P_SIZE)
            pdf.multi_cell(0, SMALL_LINE_HEIGHT + BOX_PADDING, f"Space utilised: {spaceUtilized}")

            pdf.multi_cell(0, SMALL_LINE_HEIGHT + BOX_PADDING, f"Left-most position at {datetime.timedelta(seconds=round(min_frame/fps,0))} (hours:minutes:seconds)")

            pdf.multi_cell(0, SMALL_LINE_HEIGHT + BOX_PADDING, f"Right-most position at {datetime.timedelta(seconds=round(max_frame/fps,0))} (hours:minutes:seconds)")
            ...
            pdf.set_font("Arial", "I", P_SIZE - 2)
            pdf.multi_cell(0, SMALL_LINE_HEIGHT, "Figure 3: Wrist positions over time")
            ...
            if len(left_gestures_over_limit) != 0:
                for gesture in left_gestures_over_limit:
                    ...
                    pdf.multi_cell(0, SMALL_LINE_HEIGHT+BOX_PADDING, f"The gesture from {str(datetime.timedelta(seconds=round(float(gesture[0]))))} to {str(datetime.timedelta(seconds=round(float(gesture[1]))))} exceeded the 5-second recommended time frame")
            else:
                pdf.multi_cell(0, SMALL_LINE_HEIGHT+BOX_PADDING, f"No left hand gestures exceeding the 5-second recommended time frame detected. Yay!")
            ...
            if len(right_gestures_over_limit) != 0:
                for gesture in right_gestures_over_limit:
                    pdf.multi_cell(0, SMALL_LINE_HEIGHT+BOX_PADDING, f"The gesture from {str(datetime.timedelta(seconds=round(float(gesture[0]))))} to {str(datetime.timedelta(seconds=round(float(gesture[1]))))} exceeded the 5-second recommended amount")
            else:
                pdf.multi_cell(0, SMALL_LINE_HEIGHT+BOX_PADDING, f"No right hand gestures exceeding the 5-second recommended time frame detected. Yay!")
        ...
        print(f'alertmessage: {alertMessage}')
